...
void rmusr(std::vector<std::string> &parametros, std::vector<disco> &discos, usuario &sesion){
    ...
    if(!existe_usuario){
        std::cout << "ERROR: El usuario que desea eliminar no existe." << std::endl;
        fclose(archivo);
        return;
    }
    ...
    std::cout << "MENSAJE: Usuario eliminado correctamente." << std::endl;
    fclose(archivo);
}
